"""Reliable Internet Archive uploader using the official internetarchive client."""
...
class IAUploader:
    """IA publisher with explicit credentials, retries and client-side verification."""
    PREFLIGHT_TTL = 60.0
    ...
    def __init__(self, access_key: str, secret_key: str, identifier: str,
        ...
        self.access_key = access_key.strip(); self.secret_key = secret_key.strip()
        self.identifier = identifier.strip(); self.endpoint_url = endpoint_url.rstrip("/")
    ...
    def check_collection(self, force=False):
        ...
        if not exists: result=(False, f"COLLECTION_NOT_FOUND: {self.collection}" if msg=="NOT_FOUND" else f"COLLECTION_CHECK_FAILED: {msg}")
        else:
        ...
        self._collection_cache=(now,result[0],result[1]); return result
    ...
    def check_item(self, force=False):
        ...
        result=(True,f"ITEM_FOUND: {self.identifier}") if exists else ((False,f"ITEM_NOT_FOUND: {self.identifier}") if msg=="NOT_FOUND" else (False,f"ITEM_CHECK_FAILED: {msg}"))
        self._item_cache=(now,result[0],result[1]); return result

    def invalidate_item_cache(self): self._item_cache=None
    ...
    def _metadata(self):
        ...
        if self.title:md["title"]=self.title
        if self.creator:md["creator"]=self.creator
        if self.description:md["description"]=self.description
        return md
    ...
    def upload_file_s3(self, local_path: Path | str, remote_key: str, content_type="application/octet-stream", metadata_headers=None):
        ...
        if not ok:return False,msg
        headers=dict(metadata_headers or {}); headers.setdefault("Content-Type",content_type or "application/octet-stream")
        # Use the official IA uploader rather than raw PUT. It supplies the upload
        # headers/body correctly and supports retries + checksum verification.
        try:
            item=internetarchive.get_item(self.identifier, config={"s3": {"access": self.access_key, "secret": self.secret_key}})
            result=item.upload_file(
                str(local_p), key=remote_key, metadata=self._metadata(), headers=headers,
                access_key=self.access_key, secret_key=self.secret_key,
                queue_derive=False, verify=True, checksum=True, retries=8, retries_sleep=3,
                verbose=False,
            )
            if result is True or result:
                self._item_cache=(time.monotonic(),True,f"ITEM_FOUND: {self.identifier}")
                return True,f"ITEM_EXISTS/CREATED: {self.identifier} • UPLOAD_SUCCESS"
    ...
    def _client(self):
        return boto3.client("s3",endpoint_url=self.endpoint_url,aws_access_key_id=self.access_key,aws_secret_access_key=self.secret_key,region_name="us-east-1")
    ...
    def verify_remote_object(self, remote_key, expected_size, retries=5):
        for attempt in range(max(1,retries)):
            try:
                ...
                if size==expected_size:return True
...
class SpoolPublisher:
    def __init__(self,uploader,store):self.uploader,self.store=uploader,store
    def process_pending_uploads(self):
        stats={"success":0,"failed":0,"skipped":0}
        for item in self.store.get_pending_uploads():
            p=Path(item["local_path"])
            if not p.exists():self.store.update_upload_spool_status(item["id"],"failed","Local spool file missing");stats["failed"]+=1;continue
            self.store.update_upload_spool_status(item["id"],"uploading")
            ok,msg=self.uploader.upload_file_s3(p,item["remote_key"])
            if ok and self.uploader.verify_remote_object(item["remote_key"],p.stat().st_size):self.store.update_upload_spool_status(item["id"],"verified");stats["success"]+=1
            else:self.store.update_upload_spool_status(item["id"],"failed",msg);stats["failed"]+=1
        return stats
