...
class AWECrawler:
    ...
    def __init__(self, seeds: list[str], policy: CrawlPolicy, on_event=None, output_dir=None):
        ...
        core_policy = CorePolicy(
            user_agent="AWEC/12.0 (+https://github.com/ARARAT33/AWEC; archival crawler)",
    ...
    def _same_site(self, host: str, seed_host: str) -> bool:
        ...
        seed_host = self._registrable_host(seed_host)
    ...
    def _should_queue(self, url: str, mime_hint: str = "", discovery_type: str = "") -> bool:
        if not self._in_scope(url, discovery_type):
            self.stats["rejected"] += 1
            return False
        if not self.policy.download_files and discovery_type not in {"html_link", "form", "meta_refresh"}:
            return False
        return True
    ...
    async def _seed_bootstrap(self):
        ...
        for seed in self.seeds:
            ...
            for candidate, kind in ((origin + "/robots.txt", "robots"), (origin + "/sitemap.xml", "sitemap")):
                if self._in_scope(candidate):
                    self.frontier.add_url(candidate, depth=0, parent_url=seed, discovery_type=kind)
    ...
    async def _fetch_one(self, item):
        ...
        for ext_url, disc_type, mime_hint in discovered:
            if self._should_queue(ext_url, mime_hint, disc_type):
                ...
                self.stats["enqueued"] += 1
        self.on_event("discovery", {"url": res.final_url, "found": len(discovered), "queued": self.stats["enqueued"], "rejected": self.stats["rejected"]})
