...
class ResumableAWECrawler(BaseCrawler):
    """Base crawler plus durable resume, storage guard and live IA publication."""
    ...
    async def _fetch_one(self, item):
        ...
        if not (200 <= res.status < 400):
            self.stats["errors"] += 1; self.stats["retries"] += 1
        ...
        self.stats["discovered"] = self.stats.get("discovered", 0) + len(discovered)
        for ext_url, disc_type, mime_hint in discovered:
            if self._should_queue(ext_url, mime_hint, disc_type):
                ...
                # Page navigation increases depth; embedded assets do not consume page depth.
