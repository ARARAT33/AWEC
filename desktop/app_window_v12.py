...
class AWECMainWindow(V11MainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("AWEC v12 • Liquid Archive Command Center")

    def _dashboard(self):
        super()._dashboard()
        page = self.pages.widget(0); layout = page.layout()
        box = QGroupBox("♻️ Resume Center")
        bl = QVBoxLayout(box)
        self.resume_list = QListWidget(); self.resume_list.setMinimumHeight(90); bl.addWidget(self.resume_list)
        rr = QHBoxLayout(); rb = QPushButton("♻️ Resume Selected"); rb.setObjectName("primaryButton"); rb.clicked.connect(self._resume_selected); rr.addWidget(rb)
    ...
    def _crawler(self):
        ...
        f=QFormLayout(box)
        self.v12_tmp=QLabel(str(Path.home()/"AWEC"/"tmpcrawl")); self.v12_tmp.setWordWrap(True)
        choose=QPushButton("Choose…"); choose.clicked.connect(self._choose_tmp); f.addRow("Temporary folder", self.v12_tmp); f.addRow("",choose)
        self.v12_limit=QSpinBox(); self.v12_limit.setRange(0, 1024*1024); self.v12_limit.setValue(2048); self.v12_limit.setSuffix(" MB"); f.addRow("TMP limit (0 = unlimited)",self.v12_limit)
        ...
        self.v12_auto=QCheckBox("Auto-resume interrupted crawls"); self.v12_auto.setChecked(True); f.addRow(self.v12_auto)
        layout.addWidget(box)
        ...
        self.v12_verify=QCheckBox("Verify remote object size after upload"); self.v12_verify.setChecked(True); f2.addRow(self.v12_verify)
        note=QLabel("The fetched bytes are mirrored to TMPCRAWL first, then published to the configured IA item. This avoids waiting for the entire crawl to finish."); note.setWordWrap(True); f2.addRow(note)
        layout.addWidget(box2)

    def _build_archive_page(self):
        p=QWidget(); l=QVBoxLayout(p); l.setContentsMargins(24,20,24,20)
        ...
        sub=QLabel("Browse the site mirror produced by AWEC, inspect downloaded resources and jump to the Internet Archive item."); sub.setObjectName("pageSubtitle"); l.addWidget(sub)
        split=QSplitter(Qt.Orientation.Horizontal)
        self.archive_tree=QTreeWidget(); self.archive_tree.setHeaderLabels(["Downloaded site / resource"]); self.archive_tree.itemDoubleClicked.connect(self._preview_selected)
        split.addWidget(self.archive_tree)
        right=QWidget(); rl=QVBoxLayout(right); self.archive_info=QLabel("Select a crawl or file."); self.archive_info.setWordWrap(True); rl.addWidget(self.archive_info)
        ...
        split.addWidget(right); split.setSizes([420,800]); l.addWidget(split,1); self.pages.addWidget(p); self._archive_page=p
    ...
    def _load_v12_settings(self):
        ...
        if hasattr(self,'v12_tmp'): self.v12_tmp.setText(self._v12.get('tmpcrawl_dir',self.v12_tmp.text()))
        if hasattr(self,'v12_limit'): self.v12_limit.setValue(int(self._v12.get('max_local_storage_mb',2048)))
        if hasattr(self,'v12_reserve'): self.v12_reserve.setValue(int(self._v12.get('min_free_space_mb',2048)))
        if hasattr(self,'v12_keep'): self.v12_keep.setChecked(bool(self._v12.get('keep_local_mirror',True)))
        if hasattr(self,'v12_purge'): self.v12_purge.setChecked(bool(self._v12.get('purge_local_files_after_upload',False)))
        if hasattr(self,'v12_auto'): self.v12_auto.setChecked(bool(self._v12.get('auto_resume',True)))
        if hasattr(self,'v12_live'): self.v12_live.setChecked(bool(self._v12.get('archive_upload_live',True)))
        if hasattr(self,'v12_verify'): self.v12_verify.setChecked(bool(self._v12.get('archive_verify_uploads',True)))

    def _save_v12_settings(self):
        self._v12={'tmpcrawl_dir':self.v12_tmp.text(),'max_local_storage_mb':self.v12_limit.value(),'min_free_space_mb':self.v12_reserve.value(),'keep_local_mirror':self.v12_keep.isChecked(),'purge_local_files_after_upload':self.v12_purge.isChecked(),'auto_resume':self.v12_auto.isChecked(),'archive_upload_live':self.v12_live.isChecked(),'archive_verify_uploads':self.v12_verify.isChecked()}
    ...
    def _cfg(self):
        ...
        # v12's chosen temporary directory is the crawler's authoritative root.
    ...
    def _resume_selected(self):
        ...
        self._v12['resume_dir']=path; self._save_v12_settings()
        self._log(f'♻️ Resume requested: {path}')
        self.start_crawl()
    ...
    def _refresh_resume_list(self):
        ...
        for x in find_resumable_crawls(root):
            text=f"{x['crawl_id']} • pending={x['counts'].get('pending',0):,} • in-progress={x['counts'].get('in_progress',0):,}"
            it=__import__('PySide6.QtWidgets',fromlist=['QListWidgetItem']).QListWidgetItem(text); it.setData(Qt.ItemDataRole.UserRole,x['path']); self.resume_list.addItem(it)
    ...
    def _refresh_archive_explorer(self):
        ...
        for site in sorted(root.glob('crawls/*/site'), key=lambda p:p.parent.stat().st_mtime, reverse=True):
            top=QTreeWidgetItem([f"🗂 {site.parent.name} • {site}"]); top.setData(0,Qt.ItemDataRole.UserRole,str(site)); self.archive_tree.addTopLevelItem(top)
            count=0
            try:
                for f in site.rglob('*'):
                    if f.is_file():
